...
-- | Unified parser over @Thread (,) (Kleisli m)@, with the stream as ambient
-- state.
--
...
-- | Parser syntax: a @Thread (,)@ morphism in @Kleisli m@ with the stream @f@
-- as ambient state, unit input, and @These a f@ output.
newtype Parser m f s a = Parser
  { unParser :: Thread (,) (Kleisli m) f () (These a f)
  }
...
runParser p f = snd <$> runKleisli (runThread (unParser p)) (f, ())
...
satisfy p = Parser $ Thread $ Kleisli $ \(f, ()) ->
...
filterP (Parser p) f = Parser $ Thread $ Kleisli $ \(s, ()) -> do
  (_, r) <- runKleisli (runThread p) (s, ())
...
endOfInput = Parser $ Thread $ Kleisli $ \(f, ()) ->
...
instance (Monad m) => Functor (Parser m f s) where
  fmap g (Parser p) = Parser $ Thread $ Kleisli $ \(f, ()) -> do
    (f', r) <- runKleisli (runThread p) (f, ())
    pure (f', first g r)

instance (Monad m, Uncons f s) => Applicative (Parser m f s) where
  pure a = Parser $ Thread $ Kleisli $ \(f, ()) -> pure (f, These a f)

  Parser pf <*> Parser pa = Parser $ Thread $ Kleisli $ \(s, ()) ->
    let app g s' = do
          (s2, r2) <- runKleisli (runThread pa) (s', ())
          case r2 of
...
instance (Monad m, Uncons f s) => Monad (Parser m f s) where
  Parser m >>= k = Parser $ Thread $ Kleisli $ \(s, ()) -> do
    (s1, r1) <- runKleisli (runThread m) (s, ())
    case r1 of
      That _ -> pure (s1, That s1)
      This a -> runKleisli (runThread (unParser (k a))) (nil @f @s, ())
      These a s1' -> runKleisli (runThread (unParser (k a))) (s1', ())

instance (Monad m, Uncons f s) => Alternative (Parser m f s) where
  empty = Parser $ Thread $ Kleisli $ \(f, ()) -> pure (f, That f)

  Parser p1 <|> Parser p2 = Parser $ Thread $ Kleisli $ \(s, ()) -> do
    (s1, r1) <- runKleisli (runThread p1) (s, ())
    case r1 of
      That _ -> runKleisli (runThread p2) (s1, ())
      _ -> pure (s1, r1)
...
takeRest = Parser $ Thread $ Kleisli $ \(s, ()) ->
...
try (Parser p) = Parser $ Thread $ Kleisli $ \(s, ()) -> do
  (_, r) <- runKleisli (runThread p) (s, ())
  pure $ case r of
    That _ -> (s, That s)
...
capturedBS (Parser p) = Parser $ Thread $ Kleisli $ \(s, ()) -> do
  (_, r) <- runKleisli (runThread p) (s, ())
  pure $ case r of
    That _ -> (s, That s)
...
span p = Parser $ Thread $ Kleisli $ \(s, ()) ->
  let r = go [] s
   in pure (leftoverOf @f @s r, r)
  where
...
span1 p = Parser $ Thread $ Kleisli $ \(s, ()) ->
